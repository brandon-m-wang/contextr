    ...
    firebase.auth().onAuthStateChanged(async function (user) {
        if (user) {
            ...
            await firebase.firestore().collection('userspublic').doc(userID).get().then(async function (doc) {
                var notifs = await doc.data().notifications
                for (let i = 0; ((i < notifs.length && i < 10) || i < 10); i++){
                    await firebase.firestore().collection('posts').doc(notifs[i]).get().then(async function (doc){
                        var formattedDate = await doc.data().dateFormatted
                        var writer = await doc.data().poster
                        var theme = await doc.data().theme
